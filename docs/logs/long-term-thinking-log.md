...
- Success definition:
  ...
  - The extracted seams cover at least part of provider/catalog startup, persistence/cron startup, or webhook/server assembly.
...
- Next verification step: add failing tests for the first bootstrap helpers, implement the minimal extraction, then rerun `go test ./cmd/harnessd` before the full regression gate.
