...
- Conclusion: ownership remains token-fenced at the store boundary; an ordinary contender can claim only pending/retry work after the previous live owner has returned and released its token. Every filesystem-backed durable manager joins the sidecar `flock` before Set or dispatch. Current-version dispatch tokens carry a private fenced marker; startup recovery may reclaim only such a token after kernel process-loss authority. An expired unmarked (older/unfenced) token fails closed because wall clock and a new lock do not prove its live process died.
...
- Compatibility: existing rows and token-fenced terminal transitions remain compatible; nullable lease fields recover only for a current fenced owner while the workspace recovery fence is held. Unmarked pre-fence dispatch rows are deliberately preserved rather than guessed dead.
- Mixed versions: a newer manager refuses automatic takeover of an older/unfenced live owner, even if its timestamp has expired. Confirmed crash recovery remains defined for a current fenced process once the kernel releases its sidecar lock.

## Lifecycle, Security, and Reliability

- Concurrency: every pooled SQLite connection receives WAL and busy timeout; claim/reclaim success is verified by both ID and caller token. Claim errors get an eventual, capped exponential local rearm without consuming the admission budget. At deadline, the admission returns before `ReleaseLease` atomically clears its token to `retry_wait` with a safe visible reason; ordinary timers do not reclaim expired `dispatching` rows.
