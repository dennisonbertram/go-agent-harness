...
| `--workspace` nonexistent path | `-workspace=/nope -prompt=hi` | forwarded, server validates | OK (fixed by issue #1372) | `resolveWorkspacePath` main.go:440-448 forwards `workspace_path`; `harness.RunRequest.WorkspacePath` is now validated (absolute, existing directory) at `StartRun` and routes file/shell tools via the per-run registry — `TestPostRunWorkspacePathNonexistentReturns400`/`TestPostRunWorkspacePathRelativeReturns400` (internal/server/http_workspace_path_test.go), `TestStartRun_WorkspacePathRootsToolsAndSandboxConfinement` (internal/harness/workspace_path_test.go) |
...
| attached image chip(s) + `/feedback [request]` | snapshot request + active run/session/transcript + bounded logs/rollouts/images without interrupting the run; copy local evidence, upload it to the GitHub feedback-asset prerelease, create the issue directly, and report its URL | OK | `feedback.go`, `feedback_context.go`; `TestExecuteFeedbackCommand_AttachedImageIsBundledAndPublishesByDefault`, `TestPublishFeedbackIssue_UploadsAssetsAndCreatesDirectIssue` |
| `/feedback --local [request]` | capture the same local bundle without GitHub publication; consume only captured image chips | OK | `feedback.go`; `TestExecuteFeedbackCommand_LocalConsumesCapturedImagesButPreservesOtherChips` |
| `/feedback --screenshot <path>` | compatibility path: validate a regular PNG/JPEG <=10 MiB; bundle bytes plus checksum/provenance; reject disguised/symlink/oversized input | OK | `feedback_context.go`; feedback_internal_test.go |
| `/feedback` GitHub failure | preserve local zip/image sidecars and retryable pending chips; do not claim an issue exists | OK | `feedback_context.go`, `model.go`; `TestPublishFeedbackIssue_ReusesReleaseAndStopsBeforeIssueOnUploadFailure`, `TestFeedbackPublishResult_ConsumesOnlyCapturedChipsOnSuccessAndKeepsAllOnFailure` |
| `/feedback` redaction | canary secrets never survive in text members or generated issue body; screenshot pixels explicitly remain unredacted | OK | feedback_internal_test.go (config, request, transcript, log, rollout, issue canaries) |
