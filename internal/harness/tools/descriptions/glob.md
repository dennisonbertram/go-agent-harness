Find files by filename pattern using glob wildcard syntax. Use this to locate files when you know part of the filename, extension, or directory name but not the exact path. Glob matches file and directory NAMES only — it does NOT search file contents (use grep for content search).
